...
export class PriceService {
  ...
  getBlockPrice$(blockTimestamp: number, singlePrice = false): Observable<Price | undefined> {
    ...
    /**
     * Query all price history only once. The observable is invalidated after 10 seconds
     */
    else {
      if (!this.priceObservable$ || (this.priceObservable$ && (now - this.lastPriceHistoryUpdate > 10))) {
