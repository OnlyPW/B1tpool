...
export class GroupPreviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.seoService.setTitle(`b1tpool.org Lightning Nodes`);
      ...
      .pipe(
        switchMap((params: ParamMap) => {
          ...
          if (this.slug === 'the-mempool-open-source-project') {
            this.groupId = 'b1tpool.org';
            this.group = {
              name: 'B1T Core Project',
              description: 'These are the Lightning nodes operated by B1T Core Project that provide data for the b1tpool.org website. Connect to us!',
            };
          } else {
            this.group = {
